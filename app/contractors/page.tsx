...
import { CONTRACTOR_PARTY_TYPES, deleteContractor, getContractors, updateContractor } from '@/lib/contractors'
...
export default async function ContractorsPage({
  ...
  const editingContractorId = Array.isArray(resolvedSearchParams.edit)
  ...
  const contractors = await getContractors(db)
  const editingContractor = contractors.find((contractor) => contractor.id === editingContractorId)

  async function updateContractorDetails(formData: FormData) {
    'use server'
  ...
  async function deleteContractorAction(formData: FormData) {
    ...
    await deleteContractor(contractorId, db)
    redirect('/contractors')
  }
  ...
  return (
    <div className="min-h-full">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-bold text-slate-900">Contractor-Type Parties</h2>
          <Link
            href="/contractors/new"
            className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800"
          >
            + New Contractor-Type Party
          </Link>
        </div>
              ...
              {contractors.map((contractor) => (
                <tr key={contractor.id} className="hover:bg-slate-50">
                  <td className="px-6 py-4">
                    ...
                    {contractor.address && <div className="text-xs text-slate-500">{contractor.address}</div>}
                  </td>
                  <td className="px-6 py-4 text-sm text-slate-600">{contractor.party_type}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">
                    <div>{contractor.contact_person || '-'}</div>
                    <div className="text-xs">{contractor.phone || '-'}</div>
                    {contractor.email && <div className="text-xs">{contractor.email}</div>}
                  </td>
                  <td className="px-6 py-4 text-sm text-slate-600">{contractor.trn_number || '-'}</td>
              ...
              {contractors.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-slate-500">
                    No contractor-type parties found. Add contractors, subcontractors, or consultants for project records.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {editingContractor && (
          <section className="mt-8 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between gap-4">
              <div>
                <h3 className="text-lg font-semibold text-slate-900">Edit Contractor-Type Party</h3>
                <p className="mt-1 text-sm text-slate-600">
                  Update contractor, subcontractor, or consultant details here.
                </p>
              </div>
              <Link
                href="/contractors"
            ...
            <form action={updateContractorDetails} className="mt-6 space-y-6">
              <input type="hidden" name="contractor_id" value={editingContractor.id} />

              <div className="grid gap-4 md:grid-cols-2">
                <Field label="Party Type" htmlFor="party_type">
                  <select id="party_type" name="party_type" defaultValue={editingContractor.party_type} className={inputClassName}>
                ...
                <Field label="Name" htmlFor="name">
                  <input id="name" name="name" type="text" required defaultValue={editingContractor.name} className={inputClassName} />
                </Field>
                <Field label="Contact Person" htmlFor="contact_person">
                  <input id="contact_person" name="contact_person" type="text" defaultValue={editingContractor.contact_person || ''} className={inputClassName} />
                </Field>
                <Field label="Phone" htmlFor="phone">
                  <input id="phone" name="phone" type="text" defaultValue={editingContractor.phone || ''} className={inputClassName} />
                </Field>
                <Field label="Email" htmlFor="email">
                  <input id="email" name="email" type="email" defaultValue={editingContractor.email || ''} className={inputClassName} />
                </Field>
                <Field label="TRN Number" htmlFor="trn_number">
                  <input id="trn_number" name="trn_number" type="text" defaultValue={editingContractor.trn_number || ''} className={inputClassName} />
                </Field>
              </div>

              <Field label="Address" htmlFor="address">
                <textarea id="address" name="address" rows={3} defaultValue={editingContractor.address || ''} className={textareaClassName} />
              </Field>

              <Field label="Notes" htmlFor="notes">
                <textarea id="notes" name="notes" rows={3} defaultValue={editingContractor.notes || ''} className={textareaClassName} />
              </Field>
